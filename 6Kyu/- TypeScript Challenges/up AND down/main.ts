...
// strng -> string of words/letters separated by spaces. Mix of lowercase & uppercase.
// return -> string rearanged by:
// length t(O) <= length t(1) >= length t(2) <= length t(3) >= length t(4)
// and then every OTHER word is uppercased
export function arrange(strng: string): string {
  const wordArr: string[] = strng.split(" ");

  for (let i = 0; i < wordArr.length - 1; i++) {
    let current: string = wordArr[i];
    let next: string = wordArr[i + 1];
...
// Other Solutions
export class G964 {
  public static arrange(strng: string): string {
    const subStrings = strng.split(" ");

    return subStrings
      .reduce((strings, curr, index) => {
        if (index + 1 === subStrings.length) {
          return strings;
        }

        switch (index % 2) {
          case 0:
            if (curr.length > strings[index + 1].length) {
              const temp = curr;
              strings[index] = strings[index + 1];
              strings[index + 1] = curr;
            }
            break;
          case 1:
            if (curr.length < strings[index + 1].length) {
